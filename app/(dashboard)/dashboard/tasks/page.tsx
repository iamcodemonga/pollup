    ...
    return (
        <main className='w-full relative overflow-x-hidden ml-14 lg:pl-3 allpolls pb-20'>
            ...
            <div className='flex w-full pr-2 py-5 mt-0'>
                {/* <h1 className='text-2xl lg:text-4xl'>Tasks</h1> */}
            </div>
            <div className='pr-2'>
                ...
                <div className='mt-4 grid lg:grid-cols-5 gap-3 mb-10'>
                    {tasks.length > 0 ? tasks.map((task, index) => <TaskCard task={task} key={index} userid={task.type == "referral" ? user.id : ""} >
                        ...
                        </svg> : task.type == "video" ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-7 text-primary">
                            <path fillRule="evenodd" d="M4.5 5.653c0-1.427 1.529-2.33 2.779-1.643l11.54 6.347c1.295.712 1.295 2.573 0 3.286L7.28 19.99c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" />
                        </svg> : task.type == "poll" ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-8 text-primary">
                            <path d="M18.375 2.25c-1.035 0-1.875.84-1.875 1.875v15.75c0 1.035.84 1.875 1.875 1.875h.75c1.035 0 1.875-.84 1.875-1.875V4.125c0-1.036-.84-1.875-1.875-1.875h-.75ZM9.75 8.625c0-1.036.84-1.875 1.875-1.875h.75c1.036 0 1.875.84 1.875 1.875v11.25c0 1.035-.84 1.875-1.875 1.875h-.75a1.875 1.875 0 0 1-1.875-1.875V8.625ZM3 13.125c0-1.036.84-1.875 1.875-1.875h.75c1.036 0 1.875.84 1.875 1.875v6.75c0 1.035-.84 1.875-1.875 1.875h-.75A1.875 1.875 0 0 1 3 19.875v-6.75Z" />
                        </svg> : task.type == "referral" ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-7 text-primary">
