...
import UserPollCard from '../dashboard/UserPollCard';
import MoonLoader from 'react-spinners/MoonLoader';
import DashboardCard from '../loader/DashboardCard';
import { useInfiniteActiveUserPolls } from '@/hooks/activeuserpolls';
import EmptyActivePolls from '../empty/EmptyActivePolls';
...
const UserActivePollsFetcher = () => {

    const { data: polls, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError } = useInfiniteActiveUserPolls();
    console.log(polls);
        ...
        <>
            <section className='w-full pb-10 allorders-section hidde pr-2 lg:pr-4 space-y-3 lg:space-y-5'>
                ...
                {polls?.pages[0].length > 0 ? polls?.pages.map((pages, pageIndex) => <div className='grid grid-cols-12 gap-x-3 gap-y-3 lg:gap-y-5 lg:gap-x-3' key={pageIndex}>
                    {pages.length > 0 ? pages.map((page: Props, index: number) => <UserPollCard data={page} key={index}/>) : null}
                </div>) : <EmptyActivePolls />}
            </section>
...
export default UserActivePollsFetcher